...
export class SearchResultComponent implements OnInit {
  ...
  filterDealsByMinDuration(deals: Array<Deal>, minDuration: number): Array<Deal> {
    ...
    filterObservable = source.filter(deal => parseInt(deal.duration.h) * 60 + parseInt(deal.duration.m) == minDuration);
  ...
  calculateArrayDurations(deals: Array<Deal>): Array<number> {
    ...
    for (let deal of deals) {
      arrayDurations.push((deal.duration.h * 60) + (deal.duration.m));
    }
